...
def sweepCurve(name, point, radius):
    ...
    cmds.extrude(name+"_profile", name, et=2, n=name+"_mesh", fpt=1, p=point, sc=0.6)
...
def createAnim(itemName, itemRotation, rotationFactor, start, stop, step):
	for i in range(start, stop, step * 2):
		...
		cmds.setKeyframe(itemName, at="rotateX", time=i + step, v=itemRotation[0] + (r.uniform(rotationFactor / 2, rotationFactor)))
		cmds.setKeyframe(itemName, at="rotateY", time=i + step, v=itemRotation[1] + (r.uniform(rotationFactor / 2, rotationFactor)))
		cmds.setKeyframe(itemName, at="rotateZ", time=i + step, v=itemRotation[2] + (r.uniform(rotationFactor / 2, rotationFactor)))
...
generateCurve(n,height, (0, 0, 0), 1)

sweepCurve(n, (0,0,0), radius)

createBranch(branchDivStart, branchDivStart / branchDec, n, Density / 2)
